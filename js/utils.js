...
(function () {
  ...
  // ── تطبيع رقم التعريف (مفتاح الربط بين الملفات) — أرقام فقط، بلا أصفار بادئة ──
  // الأصفار البادئة تسقط عند قراءة SheetJS لملف Eleve (HTML) كأرقام، فيختلف
  // «0301…» عن «301…» لنفس التلميذ. نُزيلها من الطرفين ليتطابقا.
  const normId = (v) => toLatinDigits(String(v == null ? '' : v)).replace(/\D/g, '').replace(/^0+/, '');
